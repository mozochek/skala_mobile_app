...
class CoachingFormStartTimeTextField extends StatelessWidget {
  const CoachingFormStartTimeTextField({Key key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    final startTimeOrFailure =
        context.watch<CoachingsFormBloc>().state.coaching.startTime;
    return Padding(
      padding: const EdgeInsets.fromLTRB(16.0, 0.0, 16.0, 8.0),
      child: DialogFormSelectionTextField(
        autovalidateMode: AutovalidateMode.onUserInteraction,
        controllerText: startTimeOrFailure.isValid()
            ? startTimeOrFailure.getOrCrash().formattedString
            : 'Выберите время',
        onTap: () async {
          ...
          if (pickedTime != null) {
            context.read<CoachingsFormBloc>().add(
                  CoachingsFormEvent.startTimeChanged(pickedTime),
                );
          }
        },
